...
const std::string database_filename = "techdb.dat";
...
public:
    StorageManager(bool trancate=false){
...
public:
    BufferManager(bool truncate):storage_manager(truncate), 
...
public:
    ...
    TechDB(bool truncate=false):buffer_manager(truncate){
    ...
    bool try_to_insert(int key, int value){
        ...
        for (size_t page_itr = 0; page_itr < num_pages; page_itr++) {
            ...
            auto string_field = std::make_unique<Field>("techdb");
    ...
    int getValueByIndex(int key){
        int value = index.getValue(key);
        return value;
    }

    int getValueByScan(int key){
        auto num_pages = buffer_manager.getNumPages();
        int queryRes = 0;
        for (size_t page_itr = 0; page_itr < num_pages; page_itr++) {
            auto& page = buffer_manager.getPage(page_itr);
            ...
            for (size_t slot_itr = 0; slot_itr < MAX_SLOTS; slot_itr++) {
                if (slot_array[slot_itr].empty == false){
                    ...
                    std::istringstream iss(tuple_data);
                    auto loadedTuple = Tuple::deserialize(iss);
                    int recordKey = loadedTuple->fields[0]->asInt();
                    int value = loadedTuple->fields[1]->asInt();
                    if(key==recordKey){
                        queryRes += value;
                    }
    ...
    void fillStatistic(){
        statistic.num_pages = buffer_manager.getNumPages();
        std::unordered_set<int> keys;

        //Calculate number of keys and number of items;
        for (size_t page_itr = 0; page_itr < statistic.num_pages; page_itr++) {
            auto& page = buffer_manager.getPage(page_itr);
            char* page_buffer = page->page_data.get();
            Slot* slot_array = reinterpret_cast<Slot*>(page_buffer);
            for (size_t slot_itr = 0; slot_itr < MAX_SLOTS; slot_itr++) {
        ...
        statistic.secondsPerPage = duration.count()/statistic.num_pages;
        statistic.secondsPerItem = duration.count()/statistic.num_items;

        //Calculate the index query time
        ...
        duration = end - start;
        statistic.secondsPerIndexQuery = duration.count()/keys.size();
    }

    double estimateQueryByScan(){
        double hitrate = std::min(1.0,static_cast<double>(MAX_PAGES_IN_MEMORY)/statistic.num_pages);
        double queryTime = (1-hitrate)*statistic.num_pages*statistic.secondsPerPage;
        return queryTime;
    }
...
int main() {
    // Get the start time
    {
        ...
        if(truncate){
            ...
            std::cout << "Total inserted number of items:"<< db.tuple_insertion_attempt_counter << std::endl;
    ...
    std::cout << "Building index using: " << elapsed.count() << " seconds" << std::endl;
    //Calculate statistics
    db.fillStatistic();

    srand(time(0));
    // Query using index
    std::vector<int> queryKeys;
    std::vector<double> querySeconds;
    std::vector<double> querySecondsByScan;
    for(int i=0;i<10;i++){
        int queryKey = rand()%100;
        queryKeys.push_back(queryKey);
    }
    
    for(auto& eachKey: queryKeys){
        start = std::chrono::high_resolution_clock::now();
        db.getValueByIndex(eachKey);
        end = std::chrono::high_resolution_clock::now();
        elapsed = end-start;
        querySeconds.push_back(elapsed.count());
    }
    
    //Query by scan the table
    for(auto& queryKey:queryKeys){
        start = std::chrono::high_resolution_clock::now();
        db.getValueByScan(queryKey);
        end = std::chrono::high_resolution_clock::now();
        elapsed = end - start;
        querySecondsByScan.push_back(elapsed.count());
    }

    std::cout << "**************************************************************" << std::endl;
    std::cout << "estimated query by index:" << db.statistic.secondsPerIndexQuery << std::endl;
    std::cout << "Query keys:";
    for(int i=0;i<10;i++){
    ...
    for(int i=0;i<10;i++){
        std::cout << querySeconds[i] << " ";
    }
    std::cout << std::endl;

    std::cout << "estimated query by scan:" << db.estimateQueryByScan() << std::endl;
    std::cout << "actual time by scan:";
    for(int i=0;i<10;i++){
        std::cout << querySecondsByScan[i] << " ";
    }
    std::cout << std::endl;
    std::cout << "----------------------------------------------------------------" << std::endl;
